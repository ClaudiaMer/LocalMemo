...
import torch
import numpy as np 
from local_memorization.plotting import get_next_panel_label
...
def load_class(leonardo=False): 
    # load training data
    if not leonardo:
        class_tensor = torch.load(DATA_PATH+"train.pt")[1][0]
    else:
        work_dir = os.environ.get("WORK")
        class_tensor = torch.load(work_dir +"/"
                              +DATA_PATH+"train.pt")[1][0]
    return class_tensor
...
def plot_samples_with_closest_train(
    ...
    if balanced_by_closest_label:
        ...
        selected = selected[torch.argsort(closest_labels_all[selected], descending=True)]
    ...
    if ylabel:
        label_x = bbox1.x0 - 0.008 #- 0.05 #*(sort_by=="closest_label")
